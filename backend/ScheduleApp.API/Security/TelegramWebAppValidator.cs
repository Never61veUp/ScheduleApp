...
namespace MiniApp.Server.Security;
...
public sealed class TelegramWebAppValidator
{
    ...
    private static Dictionary<string, string> ParseQueryString(string query)
    {
        ...
        // Important: Telegram initData is a querystring. For signature validation we need
        // correct URL-decoding semantics (including '+' -> space), so use ASP.NET parser.
        var parsed = QueryHelpers.ParseQuery(query);
        var result = new Dictionary<string, string>(StringComparer.Ordinal);
        foreach (var (key, values) in parsed)
        {
            // Telegram provides unique keys; take the first value.
